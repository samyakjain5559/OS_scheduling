...
void schedule(){
     ...
     for(x = 0;x < addnum; x++){ // inverting the Linked list
        ...
        while (temp != NULL) {
            ...
            if((temp->task->priority >= maxval) && (temp->task->tid >= 0)){
               maxval = temp->task->priority;
               maxnode = temp;
            }
            temp = temp->next;
        }
        maxnode->task->tid = -1;
        waittime = waittime + maxnode->task->burst;
        n[count] = waittime;
        count = count + 1;
        printf("Running task = [%s] [%d] [%d] for %d units.\n",maxnode->task->name, maxnode->task->priority, maxnode->task->burst, maxnode->task->burst);

        //delete(&newhead,maxnode->task);
     }
